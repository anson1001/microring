...
class SpectraCollection:
    ...
    @staticmethod
    def _clean_power(power: np.ndarray) -> np.ndarray:
        """
        Clean raw oscilloscope power before logarithmic normalization.

        Non-positive samples cannot be used in 10*log10(P_device/P_baseline).
        They are usually isolated acquisition glitches, so interpolate them
        from neighboring positive samples instead of turning them into -inf dB.
        """
    ...
    def normalize_by(self, baseline: Spectrum) -> "SpectraCollection":
        ...
        Raw TEK traces are power-like oscilloscope voltages, not dB. For raw
        spectra this computes:

            T_norm(dB) = 10 * log10(P_device / P_baseline)

        If a spectrum is already tagged as dB, it falls back to dB subtraction.
        ...
        result = {}
        baseline_unit = baseline.metadata.get("unit", "raw_power")
        for name, spec in self._spectra.items():
            spec_unit = spec.metadata.get("unit", "raw_power")
            if spec_unit == "raw_power" and baseline_unit == "raw_power":
                device_power = self._clean_power(spec.transmission_dB)
                baseline_power = self._clean_power(baseline.transmission_dB)
                ratio = device_power / (baseline_power + 1e-15)
                norm_db = 10.0 * np.log10(np.clip(ratio, 1e-15, None))
            else:
                norm_db = spec.transmission_dB - baseline.transmission_dB
            meta = dict(spec.metadata)
            meta["unit"] = "dB"
            meta["normalization"] = f"single_bus:{baseline.name}"
...
def record_table() -> pd.DataFrame:
    """
    Return record.csv with explicit row labels for UI/display.

    csv_row is 1-based after the header; pandas_index is what get_row() uses.
    """
    df = _record.copy()
    df.insert(0, "csv_row", np.arange(1, len(df) + 1))
    df.insert(1, "pandas_index", df.index)
    return df
...
def load_spectrum(row: pd.Series, data_length: int = DATA_LENGTH) -> Spectrum:
    ...
    """
    def tek_name(path_value, number_value) -> str:
        """Return a TEK filename stem from either path or numeric columns."""
        if pd.notna(path_value) and str(path_value).strip():
            return str(path_value).strip()
        if pd.notna(number_value):
            return f"TEK{int(float(number_value)):05d}"
        raise ValueError(
            "Record row needs either trigger/channel path or trigger/channel number."
        )

    channel_stem = tek_name(row.get("channel path"), row.get("channel"))
    trigger_stem = tek_name(row.get("trigger path"), row.get("trigger"))

    channel_path = str(row["folder"]) + channel_stem + ".csv"
    trigger_path = str(row["folder"]) + trigger_stem + ".csv"
...
def detect_resonances(
    ...
    """
    Detect candidate resonances without fitting them.

    Parameters
    ----------
    spectrum : Spectrum
        Usually a normalized dB spectrum.
    prominence_db : float
        Minimum feature prominence in dB.
    distance_pts : int
        Minimum feature spacing in samples.
    mode : {"dips", "peaks", "both"}
        TMM through-port fitting expects dips. Drop-port data often has peaks.
    """
    mode = mode.lower()
    ...
    def add(kind: str, signal: np.ndarray):
        ...
        for idx, prom in zip(indices, prominences):
            rows.append({
                ...
                "wavelength_nm": float(spectrum.wavelength[idx]),
                "transmission_dB": float(spectrum.transmission_dB[idx]),
                "prominence_db": float(prom),
            })

    if mode in {"dips", "both"}:
        add("dip", -spectrum.transmission_dB)
    if mode in {"peaks", "both"}:
        add("peak", spectrum.transmission_dB)
...
def tmm_fit(
    ...
    distil_resonances: bool = False,
    check_resonances: bool = False,
    **kwargs,
) -> pd.DataFrame:
    """
    ...
    **kwargs
    ...
    results_df = TMM.fit(
        ...
        input_already_normalized=spectrum.metadata.get("unit") == "dB",
        distil_resonances=distil_resonances,
        check_resonances=check_resonances,
        **kwargs,
    )
...
def _safe_averages(results_df: pd.DataFrame) -> dict:
    ...
    def col(name: str) -> pd.Series:
        if name in results_df.columns:
            return results_df[name]
        return pd.Series(dtype=float)

    return {
        "a":          _safe_mean(col("a")),
        "t":          _safe_mean(col("t")),
        "Qi":         _safe_mean(col("Qi"), positive_only=True),
        "Qc_single":  _safe_mean(col("Qc_single"), positive_only=True),
        "Ql":         _safe_mean(col("Ql"), positive_only=True),
    }
...
if __name__ == "__main__":
    ...
    print("\nmain_2.py loaded.")
    print("Use this module interactively, for example:")
    print("  import main_2 as m")
    print("  rows = m.query(device='V10', sub_device='single_ring_2', port='through')")
    print("  coll = m.quick_look(rows, single_bus_row=0, show=True)")
    print("\nFor a TMM smoke test on the current record.csv:")
    print("  sb = m.load_spectrum(m.get_row(0))")
    print("  spec = m.load_spectrum(m.get_row(3))")
    print("  norm = m.SpectraCollection({'test': spec}).normalize_by(sb).downsample(10)['test']")
    print("  res = m.tmm_fit(norm, run=m.Run('TMM_smoke_test'), ring_radius_um=27,")
    print("                  min_prominence_db=8, min_distance_pts=3000, fit_half_window_pts=500)")
